...
if(!server.includes('app: "ClearSay"')||!server.includes('version: "1.8.4"')) throw new Error('Missing ClearSay v1.8.4 export branding');
...
if(!server.includes('couples_live_closure_agreement: true')||!server.includes('/topic/closure/propose')||!server.includes('/topic/closure/respond')||!server.includes('closureAgreement')) throw new Error('Missing v1.8.4 structured Closure Agreement workflow');
if(!server.includes('couples_live_agreement_versioning: true')||!server.includes('rootId')||!server.includes('version:parent ? Number(parent.version||1)+1 : 1')) throw new Error('Missing v1.8.4 Agreement Versioning');
if(!server.includes('definition_collision_detection: true')||!server.includes('definition_collision')||!server.includes('definition_collisions')) throw new Error('Missing v1.8.4 Definition Collision detection');
if(!server.includes('current_behavior_historical_injury_split: true')||!server.includes('current_vs_historical')||!server.includes('current_behavior_vs_historical_injury')) throw new Error('Missing v1.8.4 current-behavior / historical-injury split');
if(!html.includes('couplesCloseResolved')||!html.includes('couplesCloseOpen')||!html.includes('couplesCloseRepair')||!html.includes('couplesClosureAgreement')) throw new Error('Missing v1.8.4 Closure Agreement UI');
if(!appJs.includes('respondCouplesClosure')||!appJs.includes('definition-collision-card')||!appJs.includes('history-split-card')||!appJs.includes('v${Number(x.version||1)}')) throw new Error('Missing v1.8.4 browser guardrail UI');
...
console.log('Knowledge check passed: ClearSay v1.8.4 premium landing introduction, emergency 911 acknowledgment language, per-launch acknowledgment, separate terms page, stronger button/dashboard contrast, 100 fallacies, 12 claim identifiers, repair/agreement reasoning, transcript mode, Self-Guided, response targets, and Couples Live with 15/30/45/60-minute sessions, 1:00 turn control, no inactive drafting, and repeated topic-focus guidance.');
