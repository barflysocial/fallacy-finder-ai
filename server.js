...
const COUPLES_MAX_ACTIVE_MS = 15 * 60 * 1000;
...
const couplesMessageSchema = {
  ...
  required: ["summary","claim_types","claim_status","reasoning_outcome","evidence_assessment","likely_fallacies","conversation_effect","topic_relevance","common_ground_added","disputed_or_unresolved","underlying_issue","agreement_signal","repair_signal","useful_next_questions","caution"]
};
...
function couplesParticipantFromRequest(s, req) {
  ...
  for (const id of ["A", "B"]) {
    if (s.participants[id]?.token === token) return { id, ...s.participants[id] };
  }
  return null;
}
...
function couplesRemainingMs(s) {
  if (!s.timerEndsAt) return COUPLES_MAX_ACTIVE_MS;
...
function publicTopic(topic) {
  ...
  return {
    id: topic.id, text: topic.text, status: topic.status, created_at: topic.createdAt, started_at: topic.startedAt || null, closed_at: topic.closedAt || topic.closed_at || null,
...
function publicCouplesState(s, participantId) {
  ...
  for (const id of ["A","B"]) {
    ...
    participants[id] = p ? { id, name: p.name, joined_at: p.joinedAt } : null;
  }
  return {
    code: s.code,
    ...
    max_active_ms: COUPLES_MAX_ACTIVE_MS,
...
async function runCouplesMessageAnalysis(s, message) {
  ...
  const response = await openai.responses.create({
    ...
    prompt_cache_key: "fallacy-finder-couples-message-v2",
    text: {
      verbosity: "low",
...
async function runCouplesOverview(s) {
  ...
  const response = await openai.responses.create({
    ...
    prompt_cache_key: "fallacy-finder-couples-overview-v2",
...
app.post("/api/couples/create", (req, res) => {
  ...
  const session = {
    ...
    createdAt: now,
    updatedAt: now,
    expiresAt: Date.now() + COUPLES_SESSION_TTL_MS,
    revision: 1,
    participants: { A: { token, name: cleanParticipantName(req.body?.name, "Person A"), joinedAt: now }, B: null },
    messages: [],
    insights: null,
    topic: { id: `t-${Date.now()}`, text: topicText, status: "awaiting_confirmation", createdAt: now, startedAt: null, closedAt: null, confirmations: { A: true, B: false }, closureVotes: { A: null, B: null } },
    topicHistory: [],
    parkingLot: [],
    timerStartedAt: null,
    timerEndsAt: null,
    sessionEndedAt: null,
    sessionEndReason: "",
    finalInsightsRequested: false
  };
  couplesSessions.set(code, session);
  res.json({ code, token, participant_id: "A", state: publicCouplesState(session, "A") });
});
...
app.post("/api/couples/join", (req, res) => {
  ...
  const token = newCouplesToken();
  const now = new Date().toISOString();
  s.participants.B = { token, name: cleanParticipantName(req.body?.name, "Person B"), joinedAt: now };
  touchCouplesSession(s);
...
app.post("/api/couples/:code/message", limiter, (req, res) => {
  ...
  if (!p) return res.status(401).json({ error: "This browser is not authorized for that session." });
  maybeExpireCouplesSession(s);
  if (s.sessionEndedAt) return res.status(409).json({ error: "This 15-minute Couples Live session has ended. Review or export the session, then start a new one if needed." });
  ...
  if (!s.timerStartedAt) {
    ...
    s.timerEndsAt = new Date(new Date(message.createdAt).getTime() + COUPLES_MAX_ACTIVE_MS).toISOString();
  }
...
app.post("/api/couples/:code/topic/confirm", (req, res) => {
  ...
  if (s.topic.confirmations.A && s.topic.confirmations.B) s.topic.status = "open";
  touchCouplesSession(s);
...
app.post("/api/couples/:code/topic/new", (req, res) => {
  ...
  if (!p) return res.status(401).json({ error: "This browser is not authorized for that session." });
  maybeExpireCouplesSession(s);
  if (s.sessionEndedAt) return res.status(409).json({ error: "The 15-minute session has ended. Start a new Couples Live session for another topic." });
  ...
  const now = new Date().toISOString();
  s.topic = { id: `t-${Date.now()}-${randomBytes(3).toString("hex")}`, text: topicText, status: "awaiting_confirmation", createdAt: now, startedAt: null, closedAt: null, confirmations: { A: false, B: false }, closureVotes: { A: null, B: null } };
  s.topic.confirmations[p.id] = true;
...
function couplesExportObject(s) {
  return {
    app: "Fallacy Finder AI",
    version: "1.4.1",
    session_code: s.code,
    created_at: s.createdAt,
    ...
    session_end_reason: s.sessionEndReason,
    participants: Object.fromEntries(["A","B"].map(id => [id, s.participants[id] ? { name: s.participants[id].name, joined_at: s.participants[id].joinedAt } : null])),
    current_topic: publicTopic(s.topic),
    topic_history: (s.topicHistory || []).map(publicTopic),
    parking_lot: s.parkingLot || [],
    messages: s.messages.map(m => ({ id:m.id, sender_id:m.senderId, sender_name:m.senderName, text:m.text, created_at:m.createdAt, analysis_status:m.analysisStatus, analysis:m.analysis || null })),
    insights: s.insights || null,
...
app.get("/api/couples/:code/export", (req, res) => {
  ...
  if (format === "txt") {
    const lines = [
      "Fallacy Finder AI — Couples Live Conversation",
      `Session: ${s.code}`,
      `Topic: ${s.topic?.text || ""}`,
      `Topic status: ${s.topic?.status || "unknown"}`,
      "",
    ...
    res.setHeader("Content-Disposition", `attachment; filename="Fallacy_Finder_${s.code}_Conversation.txt"`);
    return res.send(lines.join("\n"));
  }
  res.setHeader("Content-Type", "application/json; charset=utf-8");
  res.setHeader("Content-Disposition", `attachment; filename="Fallacy_Finder_${s.code}_Full_Session.json"`);
  res.send(JSON.stringify(couplesExportObject(s), null, 2));
});
