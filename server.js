...
function hasCreatorSuppliedInviteeInfo(body = {}) {
  const forbiddenFields = [
    "person_b_name", "participant_b_name", "invitee_name", "other_person_name", "guest_name",
    "person_b_phone", "participant_b_phone", "invitee_phone", "recipient_phone", "guest_phone",
    "person_b_email", "participant_b_email", "invitee_email", "recipient_email", "guest_email"
  ];
  return forbiddenFields.some((key) => String(body?.[key] ?? "").trim().length > 0);
}

app.post("/api/couples/create", (req, res) => {
  if (hasCreatorSuppliedInviteeInfo(req.body)) return res.status(400).json({ error: "Person B must enter their own information after opening the Couples Live invitation. Person A cannot add Person B's name, phone number, or email." });
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    couples_live_guest_session_only_invites: true,
    couples_live_guest_terms_gate: true,
    couples_live_rich_link_preview: true,
    couples_live_invitee_self_entry_only: true,
    couples_live_invite_preview_private_metadata: true,
    footer_pinned_to_page_bottom: true,
...
app.get("/couples-live", (req, res) => {
  const origin = requestOrigin(req).replace(/\/$/, "");
  const indexPath = path.join(__dirname, "public/index.html");
  let html = fs.readFileSync(indexPath, "utf8");
  const title = "ClearSay Couples Live Chat";
  const description = "Private ClearSay Couples Live Chat invitation. Confirm the session code, review the Terms & Conditions, enter your own display name, and join the session.";
  const image = `${origin}/clearsay-bridge-mark.png`;
  const canonical = `${origin}/couples-live`;
  const socialMeta = `
  ...
  <meta property="og:image:alt" content="ClearSay logo" />
  <meta property="og:url" content="${canonical}" />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="${description}" />
  <meta name="twitter:image" content="${image}" />`;
  html = html.replace("<title>ClearSay — Claim. Clarify. Connect.</title>", `<title>${title}</title>`);
