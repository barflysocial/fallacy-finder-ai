...
function publicCouplesState(s, participantId) {
  ...
  for (const id of ["A","B"]) {
    ...
    participants[id] = p ? { id, name: p.name, joined_at: p.joinedAt, consent_at: p.consentAt || null } : null;
  }
...
function makeCouplesSession({ name, topic, resolutionCriteria = "", carryover = null }) {
  ...
  const session = {
    ...
    participants: { A: { token, name: cleanParticipantName(name, "Person A"), joinedAt: now, consentAt: now }, B: null },
...
app.post("/api/couples/create", (req, res) => {
  ...
  const { session, token } = makeCouplesSession({ name:req.body?.name, topic:topicText, resolutionCriteria:req.body?.resolution_criteria });
...
app.post("/api/couples/join", (req, res) => {
  ...
  const token = newCouplesToken();
  const now = new Date().toISOString();
  s.participants.B = { token, name: cleanParticipantName(req.body?.name, "Person B"), joinedAt: now, consentAt: now };
  touchCouplesSession(s);
...
app.post("/api/couples/:code/end", (req, res) => {
  ...
  s.sessionEndedAt=new Date().toISOString(); s.sessionEndReason=`ended_early_by_${p.id}`; s.timerEndsAt=s.sessionEndedAt;
...
app.post("/api/couples/:code/continue", (req, res) => {
  ...
  const made=makeCouplesSession({name:p.name,topic:topic.text,resolutionCriteria:topic.resolutionCriteria||"",carryover});
  const now=new Date().toISOString();
