...
app.post("/api/couples/:code/topic/close", (req, res) => {
  ...
  if(!isCoupleParticipant(p))return res.status(403).json({error:"Only the two participants can manage topics."});
  return res.status(409).json({error:"ClearSay v1.8.4 requires a structured Closure Agreement. Propose what is resolved, what remains open, the next step, and repair completion instead of submitting a status vote."});
});
