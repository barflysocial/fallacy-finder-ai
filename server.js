...
- Time does not erase whether an event happened, but it can change how relevant that event is to a present claim. Distinguish historical context, continuing pattern, ongoing consequence, unresolved repair, and an old event being used as proof of current behavior or character.
- When the claim is specifically about repair, evaluate repair actions separately from continuing emotion: acknowledgment, responsibility where appropriate, concrete correction/restitution, behavior change, new agreements, ongoing consequences, and whether resolution is mutual or disputed. An apology alone does not prove complete repair, and continuing pain alone does not prove no repair occurred.
...
function buildClaimQualifiers(claimTypes) {
  ...
  if (isRepairClaim) {
    for (const q of (claimQualifiers.repair_status || []).slice(0, 5)) {
...
const outputSchema = {
  ...
  required: ["status","summary","claim_parts","reasoning_bridge","established_points","unresolved_points","questions","likely_fallacies","temporal_relevance","repair_status","burden_of_proof","better_wording","suggested_response","caution"]
};
...
function validateBody(body) {
  ...
  const answers = Array.isArray(body.answers) ? body.answers.slice(0, 14).map(a => ({
...
app.post("/api/route", (req, res) => {
  ...
  res.json({
    ...
    qualifying_questions: buildClaimQualifiers(claimTypes),
...
async function runAnalysis(v, candidateLimit = CANDIDATE_LIMIT, maxOutput = 1350) {
  ...
  const response = await openai.responses.create({
    ...
    prompt_cache_key: "fallacy-finder-core-v5",
    text: {
      verbosity: "low",
      format: {
        type: "json_schema",
