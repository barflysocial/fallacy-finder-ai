# ClearSay v1.8.2 — Conversation Intelligence
...
## Durable session recovery (v1.8.2)
...
- Cross-device recovery still requires the participant's existing session token. v1.8.2 does not add accounts or password-based recovery.
...
v1.8.2 keeps a blocking legal-notice gate before any app workflow can be initialized and now requires acknowledgment on every fresh app launch.
