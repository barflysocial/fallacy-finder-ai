...
function currentLegalAcknowledgement(){
  const records=loadClearSayRecords();
  return records.find(r=>r?.record_type==='legal_acknowledgement'&&r?.disclaimer_version===LEGAL_DISCLAIMER_VERSION)||null;
}
...
function saveLegalAcknowledgement(){
  ...
  const prior=existing.find(r=>r?.record_type==='legal_acknowledgement'&&r?.disclaimer_version===LEGAL_DISCLAIMER_VERSION);
  ...
  // On a new ClearSay ledger, the legal acknowledgement is record #1.
  // If a future disclaimer version requires re-acknowledgment, preserve the original first record and append the new version.
  const hasAnyLegalAck=existing.some(r=>r?.record_type==='legal_acknowledgement');
  const next=existing.length===0?[ack]:(hasAnyLegalAck?[...existing,ack]:[ack,...existing]);
...
function setLegalGate(review=false){
  ...
  gate.classList.toggle('legal-review-mode',legalReviewMode);
  document.body.classList.add('legal-pending');
  const ack=currentLegalAcknowledgement();
  if($('legalRecordNote')){
    $('legalRecordNote').textContent=ack?`Acknowledged ${new Date(ack.accepted_at).toLocaleString()} • record #${ack.sequence||1}`:'Your acknowledgment is saved as the first ClearSay activity record on this device.';
    $('legalRecordNote').classList.toggle('accepted',Boolean(ack));
  }
  if($('legalAckCheck'))$('legalAckCheck').checked=false;
  if($('legalAcknowledgeBtn'))$('legalAcknowledgeBtn').disabled=true;
}
...
function acknowledgeLegalNotice(){
  if(!$('legalAckCheck')?.checked)return;
...
document.addEventListener('DOMContentLoaded',()=>{
  $('legalAckCheck')?.addEventListener('change',()=>{if($('legalAcknowledgeBtn'))$('legalAcknowledgeBtn').disabled=!$('legalAckCheck').checked;});
  $('legalAcknowledgeBtn')?.addEventListener('click',acknowledgeLegalNotice);
  $('legalCloseBtn')?.addEventListener('click',()=>{if(currentLegalAcknowledgement())hideLegalGate();});
  $('reviewLegalBtn')?.addEventListener('click',()=>setLegalGate(true));
  if(currentLegalAcknowledgement()){hideLegalGate();initializeClearSayApp();}
  else setLegalGate(false);
});
