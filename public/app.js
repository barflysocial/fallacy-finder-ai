...
function captureCouplesInviteFromUrl(){
  try{
    ...
    if(code && (!mode || mode==='couples'))sessionStorage.setItem(COUPLES_INVITE_KEY,code);
  }catch{}
}

function pendingCouplesInviteCode(){
  try{return cleanCouplesInviteCode(sessionStorage.getItem(COUPLES_INVITE_KEY)||'');}catch{return '';}
}
...
function couplesInviteUrl(code=couplesSession.code||$('couplesRoomCode')?.textContent||''){
  ...
  url.searchParams.set('mode','couples');
...
function couplesInviteMessage(){
  ...
  return `Join me in a ClearSay Couples Live session.\n\nOpen this link: ${link}\n\nSession code: ${code}\n\nThe link will open Couples Live and fill in the code automatically.`;
}
...
function initializeClearSayApp(){
  ...
  if(hasPendingCouplesInvite)applyPendingCouplesInvite();
}
