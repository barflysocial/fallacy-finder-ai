...
const COUPLES_SESSION_KEY='fallacyFinderCouplesLiveV152';
const COUPLES_INVITE_KEY='clearsayPendingCouplesInviteV191';
const COUPLES_GUEST_KEY='clearsayCouplesGuestV193';
...
const THEME_KEY='clearSayThemeV186';
let billingState={enabled:false,required:false,configured:false,access_active:true,expires_at:null,price_cents:0,currency:'usd',access_hours:0,product_name:'ClearSay Access'};
let transcriptSaveTimer=null;
let screenshotUploads=[];
...
let couplesGuestInviteMode=false;
let couplesGuestInviteCode='';
...
function setStatementUpdateNotice(kind=''){
  const badge=$('statementUpdateBadge');
  const modeNotice=$('statementModeNotice');
  const labels={qualification:'New questions below',clarification:'More information needed',analysis:'New analysis below'};
  const label=labels[kind]||'';
  if(badge){badge.textContent=label;badge.classList.toggle('hidden',!label);badge.dataset.kind=kind||'';}
  if(modeNotice){modeNotice.textContent=kind==='clarification'?'Action':'New';modeNotice.classList.toggle('hidden',!label);modeNotice.dataset.kind=kind||'';}
}
function collapseStatementPanelForUpdate(kind){
  const panel=$('statementMode');
  if(panel&&'open' in panel)panel.open=false;
  setStatementUpdateNotice(kind);
}
...
function resolvedTheme(pref='system'){
  if(pref==='light'||pref==='dark')return pref;
  return window.matchMedia?.('(prefers-color-scheme: dark)').matches?'dark':'light';
}
function applyTheme(pref='system',persist=true){
  const safe=['system','light','dark'].includes(pref)?pref:'system';
  const actual=resolvedTheme(safe);
  document.documentElement.dataset.theme=actual;
  document.documentElement.style.colorScheme=actual;
  const meta=document.querySelector('meta[name="theme-color"]');
  if(meta)meta.setAttribute('content',actual==='dark'?'#08131d':'#0B3D91');
  if($('themeSelect'))$('themeSelect').value=safe;
  if(persist){try{localStorage.setItem(THEME_KEY,safe);}catch{}}
}
function initTheme(){
  let pref='system';
  try{pref=localStorage.getItem(THEME_KEY)||'system';}catch{}
  applyTheme(pref,false);
  $('themeSelect')?.addEventListener('change',e=>applyTheme(e.target.value,true));
  window.matchMedia?.('(prefers-color-scheme: dark)').addEventListener?.('change',()=>{
    let current='system';try{current=localStorage.getItem(THEME_KEY)||'system';}catch{}
    if(current==='system')applyTheme('system',false);
  });
}

function formatMoney(cents=0,currency='usd'){
  try{return new Intl.NumberFormat(undefined,{style:'currency',currency:String(currency||'usd').toUpperCase()}).format(Number(cents||0)/100);}catch{return `$${(Number(cents||0)/100).toFixed(2)}`;}
}
function setPaymentMessage(message='',isError=false){
  const el=$('paymentMessage');if(!el)return;el.textContent=message;el.classList.toggle('payment-error',Boolean(isError));
}
function renderPaymentGate(){
  ...
  if($('paymentPrice'))$('paymentPrice').textContent=billingState.price_cents?formatMoney(billingState.price_cents,billingState.currency):'Payment setup pending';
  if($('paymentDuration'))$('paymentDuration').textContent=billingState.access_hours?`${billingState.access_hours}-hour access`:'Configured access';
  if($('paymentGateLead'))$('paymentGateLead').textContent=billingState.product_name?`Purchase ${billingState.product_name} to continue.`:'Complete secure checkout to continue.';
  if($('checkoutBtn'))$('checkoutBtn').disabled=!billingState.configured;
  if(!billingState.configured)setPaymentMessage('Payments are enabled, but Stripe or pricing is not fully configured yet.',true);
}
function showClearSayApp(){
...
async function fetchBillingStatus(){
  try{
    const r=await fetch('/api/billing/status',{headers:{'Accept':'application/json'}});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'Unable to check access.');
    billingState={...billingState,...d};
    return billingState;
  }catch(e){
    billingState={...billingState,enabled:false,required:false,configured:false,access_active:true};
    console.warn('Billing status unavailable; continuing without a payment gate.',e);
    return billingState;
  }
}
async function confirmReturnedPayment(){
  const params=new URLSearchParams(location.search);
  if(params.get('payment')!=='success'||!params.get('session_id'))return false;
  try{
    const sessionId=params.get('session_id');
    const r=await fetch(`/api/billing/confirm?session_id=${encodeURIComponent(sessionId)}`,{headers:{'Accept':'application/json'}});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'Payment could not be verified.');
    history.replaceState({},document.title,location.pathname);
    setPaymentMessage('Payment confirmed. Access is active.');
    return true;
  }catch(e){
    history.replaceState({},document.title,location.pathname);
    setPaymentMessage(e.message||String(e),true);
    return false;
  }
}
async function enterAfterLegal(){
  const returned=await confirmReturnedPayment();
  const state=await fetchBillingStatus();
  if(returned){const refreshed=await fetchBillingStatus();Object.assign(state,refreshed);}
  const params=new URLSearchParams(location.search);
  if(params.get('payment')==='cancelled'){
    history.replaceState({},document.title,location.pathname);
    setPaymentMessage('Checkout was cancelled. No charge was completed.');
  }
  if(state.enabled&&state.required&&!state.access_active){renderPaymentGate();return;}
  showClearSayApp();
}
async function beginCheckout(){
  const btn=$('checkoutBtn');if(btn){btn.disabled=true;btn.textContent='Opening secure checkout…';}
  setPaymentMessage('');
  try{
    ...
    if(!d.url)throw new Error('Checkout URL was not returned.');
...
async function refreshPaymentAccess(){
  const state=await fetchBillingStatus();
  if(state.access_active){showClearSayApp();return;}
  renderPaymentGate();
  setPaymentMessage('Access is not active on this browser yet. Complete checkout, then return here.');
}
...
function openHistoryItem(index){
  ...
  collapseStatementPanelForUpdate('analysis');
...
// -----------------------------------------------------------------------------
function cleanCouplesInviteCode(value=''){
  return String(value||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,8);
}

function captureCouplesInviteFromUrl(){
  try{
    const params=new URLSearchParams(location.search);
    const code=cleanCouplesInviteCode(params.get('code')||params.get('session')||'');
    const mode=String(params.get('mode')||'').toLowerCase();
    const guestPath=/\/couples-live\/?$/i.test(location.pathname);
    const guestFlag=params.get('guest')==='1'||guestPath||Boolean(code&&mode==='couples');
    couplesGuestInviteMode=guestFlag;
    couplesGuestInviteCode=code;
    if(guestFlag){try{sessionStorage.setItem(COUPLES_GUEST_KEY,'1');}catch{}}
    if(code && (!mode || mode==='couples' || guestFlag))sessionStorage.setItem(COUPLES_INVITE_KEY,code);
  }catch{}
}

function pendingCouplesInviteCode(){
  try{return cleanCouplesInviteCode(sessionStorage.getItem(COUPLES_INVITE_KEY)||couplesGuestInviteCode||'');}catch{return couplesGuestInviteCode||'';}
}

function savedCouplesResumeMatches(code=''){
  const safe=cleanCouplesInviteCode(code);
  if(!safe)return false;
  try{
    const raw=localStorage.getItem(COUPLES_SESSION_KEY);
    const x=raw?JSON.parse(raw):null;
    return Boolean(x?.token&&cleanCouplesInviteCode(x?.code)===safe);
  }catch{return false;}
}

function guestLegalAckPayload(){
  return {accepted:true,version:LEGAL_DISCLAIMER_VERSION,accepted_at:new Date().toISOString()};
}

function setCouplesGuestMessage(message='',isError=false){
  const el=$('couplesGuestMessage');if(!el)return;
  el.textContent=message;
  el.classList.toggle('guest-error',Boolean(isError));
}

function showCouplesGuestGate(code='',message=''){
  couplesGuestInviteMode=true;
  couplesGuestInviteCode=cleanCouplesInviteCode(code||couplesGuestInviteCode||pendingCouplesInviteCode());
  $('legalGate')?.classList.add('hidden');
  $('paymentGate')?.classList.add('hidden');
  $('appShell')?.classList.add('hidden');
  $('couplesGuestGate')?.classList.remove('hidden');
  document.body.classList.add('app-locked');
  document.body.classList.remove('couples-guest-session');
  if($('couplesGuestCode'))$('couplesGuestCode').textContent=couplesGuestInviteCode||'Code unavailable';
  setCouplesGuestMessage(message,Boolean(message));
  if($('couplesGuestJoinBtn'))$('couplesGuestJoinBtn').disabled=!couplesGuestInviteCode;
}

function activateCouplesGuestShell(){
  couplesGuestInviteMode=true;
  $('couplesGuestGate')?.classList.add('hidden');
  $('legalGate')?.classList.add('hidden');
  $('paymentGate')?.classList.add('hidden');
  $('appShell')?.classList.remove('hidden');
  document.body.classList.remove('app-locked');
  document.body.classList.add('couples-guest-session');
  initializeClearSayApp();
  switchMode('couples');
}

async function joinCouplesGuestSession(){
  const code=cleanCouplesInviteCode(couplesGuestInviteCode||$('couplesGuestCode')?.textContent||pendingCouplesInviteCode());
  const name=($('couplesGuestName')?.value||'').trim();
  const terms=Boolean($('couplesGuestTerms')?.checked);
  const sharedConsent=Boolean($('couplesGuestSharedConsent')?.checked);
  if(!code){setCouplesGuestMessage('This invitation does not contain a valid session code.',true);return;}
  if(!terms){setCouplesGuestMessage('Acknowledge the Terms & Conditions before joining.',true);return;}
  if(!sharedConsent){setCouplesGuestMessage('Confirm the shared-session visibility and export notice before joining.',true);return;}
  const btn=$('couplesGuestJoinBtn');if(btn){btn.disabled=true;btn.textContent='Joining…';}
  setCouplesGuestMessage('');
  try{
    const r=await fetch('/api/couples/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,name,consent:true,legal_ack:guestLegalAckPayload()})});
    ...
    try{sessionStorage.removeItem(COUPLES_INVITE_KEY);sessionStorage.setItem(COUPLES_GUEST_KEY,'1');}catch{}
    saveCouplesResume();
    activateCouplesGuestShell();
    renderCouplesState(d.state);
    startCouplesPoll();
  }catch(e){setCouplesGuestMessage(e.message||String(e),true);}
  finally{if(btn){btn.disabled=false;btn.textContent='Join Couples Live';}}
}

function applyPendingCouplesInvite(){
  const code=pendingCouplesInviteCode();
  if(!code)return false;
  if(couplesSession.code&&couplesSession.token&&cleanCouplesInviteCode(couplesSession.code)!==code){
    stopCouplesPoll();
  ...
  if($('couplesJoinCode'))$('couplesJoinCode').value=code;
  switchMode('couples');
  if(couplesSession.code&&couplesSession.token&&cleanCouplesInviteCode(couplesSession.code)===code)return true;
  if($('couplesJoinCode'))$('couplesJoinCode').value=code;
  setTimeout(()=>$('couplesJoinName')?.focus(),0);
  return true;
}

function couplesInviteUrl(code=couplesSession.code||$('couplesRoomCode')?.textContent||''){
  const safe=cleanCouplesInviteCode(code);
  const url=new URL('/couples-live',location.origin);
  url.searchParams.set('code',safe);
  url.searchParams.set('guest','1');
  return url.toString();
}

function couplesInviteMessage(){
  const code=cleanCouplesInviteCode(couplesSession.code||$('couplesRoomCode')?.textContent||'');
  const link=couplesInviteUrl(code);
  return `ClearSay Couples Live Chat\n\nYou've been invited to a private Couples Live Chat.\n\n${link}\n\nSession code: ${code}\n\nOpen the link, confirm the code, acknowledge the Terms & Conditions, enter your own display name, and join.`;
}

function textCouplesInvite(){
  clearError();
  const code=cleanCouplesInviteCode(couplesSession.code||$('couplesRoomCode')?.textContent||'');
  if(!code){error('Create a Couples Live session first.');return;}
  const body=encodeURIComponent(couplesInviteMessage());
  const isiOS=/iPad|iPhone|iPod/.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1);
  location.href=isiOS?`sms:&body=${body}`:`sms:?body=${body}`;
}

async function copyCouplesInvite(){
  const code=cleanCouplesInviteCode(couplesSession.code||$('couplesRoomCode')?.textContent||'');
  if(!code){error('Create a Couples Live session first.');return;}
  const invite=couplesInviteMessage();
  const btn=$('couplesCopyInviteBtn');
  try{
    await navigator.clipboard.writeText(invite);
    if(btn){btn.textContent='Invite copied';setTimeout(()=>btn.textContent='Copy invite',1200);}
  }catch{window.prompt('Copy this ClearSay invite:',invite);}
}
...
function renderCouplesAnalysis(shared,status,errorText='',privateAnalysis=null){
  ...
  const def=a.definition_collision||{};
  const definition=def.applies?`<div class="couples-signal-card definition-collision-card"><strong>Definition collision — ${escapeHtml(def.term||'shared term')}</strong><p><b>Person A means:</b> ${escapeHtml(def.participant_a_meaning||'Unclear')}</p><p><b>Person B means:</b> ${escapeHtml(def.participant_b_meaning||'Unclear')}</p><p>${escapeHtml(def.assessment||'')}</p>${def.clarifying_question?`<div class="quote">${escapeHtml(def.clarifying_question)}</div>`:''}</div>`:'';
  const hist=a.current_vs_historical||{};
  const currentHistory=hist.applies?`<div class="couples-signal-card history-split-card"><strong>Current behavior vs. historical injury</strong><p><b>Historical injury:</b> ${escapeHtml(hist.historical_injury||'Unclear')}</p><p><b>Current behavior:</b> ${escapeHtml(hist.current_behavior||'Unclear')}</p><span class="type-badge">${escapeHtml(humanize(hist.relationship||'unclear'))}</span><p>${escapeHtml(hist.assessment||'')}</p></div>`:'';
...
function renderCouplesInsightsBox(insights){
  ...
  const defs=(o.definition_collisions||[]).map(x=>`<div class="couples-signal-card definition-collision-card"><strong>${escapeHtml(x.term||'Definition collision')}</strong><p><b>Person A:</b> ${escapeHtml(x.participant_a_meaning||'')}</p><p><b>Person B:</b> ${escapeHtml(x.participant_b_meaning||'')}</p><p>${escapeHtml(x.assessment||'')}</p>${x.clarifying_question?`<div class="quote">${escapeHtml(x.clarifying_question)}</div>`:''}</div>`).join('');
  const hist=o.current_behavior_vs_historical_injury||{};
  el.innerHTML=`<details class="couples-insights-details" open><summary><span>Conversation insights</span><span class="detail-hint">${escapeHtml(o.main_issue||'Underlying issue')}</span></summary><div class="live-overview-body">
    ...
    ${defs?`<h3>Definition collisions</h3>${defs}`:''}
    ${hist.applies?`<div class="couples-signal-card history-split-card"><strong>Current behavior vs. historical injury</strong><p><b>Historical injury:</b> ${escapeHtml(hist.historical_injury||'')}</p><p><b>Current behavior:</b> ${escapeHtml(hist.current_behavior||'')}</p><span class="type-badge">${escapeHtml(humanize(hist.relationship||'unclear'))}</span><p>${escapeHtml(hist.assessment||'')}</p></div>`:''}
...
function renderCouplesTopic(state){
  ...
  const closure=topic.closure_agreement;
  if($('couplesClosureAgreement')){
    if(closure){
      const mine=closure.responses?.[me];const canRespond=canManageTopic&&topic.status==='open'&&!mine;
      $('couplesClosureAgreement').innerHTML=`<div class="closure-agreement-card"><div class="agreement-meta"><span class="type-badge">${escapeHtml(humanize(closure.status||'pending'))}</span><span class="type-badge">${escapeHtml(topicStatusLabel(closure.proposed_status||''))}</span><span class="type-badge">Repair: ${escapeHtml(humanize(closure.repair_completion||'unclear'))}</span></div><p><strong>Resolved:</strong> ${escapeHtml(closure.resolved_summary||'Nothing specified')}</p><p><strong>Still open:</strong> ${escapeHtml(closure.remains_open||'Nothing specified')}</p><p><strong>Next step:</strong> ${escapeHtml(closure.next_step||'No next step specified')}</p>${canRespond?`<div class="agreement-actions"><button class="mini-btn closure-response" data-decision="agree">Agree to this closure</button><button class="mini-btn closure-response" data-decision="disagree">Disagree</button></div>`:''}</div>`;
      document.querySelectorAll('.closure-response').forEach(btn=>btn.addEventListener('click',()=>respondCouplesClosure(btn.dataset.decision)));
    }else $('couplesClosureAgreement').innerHTML='<p class="muted">No Closure Agreement proposed yet.</p>';
  }
  if($('couplesClosureVotes')){
    const nameA=state?.participants?.A?.name||'Person A',nameB=state?.participants?.B?.name||'Person B';
    $('couplesClosureVotes').innerHTML=[a?`<div class="closure-vote-row"><strong>${escapeHtml(nameA)}:</strong> ${escapeHtml(topicStatusLabel(a.status))}${a.note?` — ${escapeHtml(a.note)}`:''}</div>`:'',b?`<div class="closure-vote-row"><strong>${escapeHtml(nameB)}:</strong> ${escapeHtml(topicStatusLabel(b.status))}${b.note?` — ${escapeHtml(b.note)}`:''}</div>`:''].filter(Boolean).join('');
  }
...
function agreementKindLabel(kind='general'){return ({general:'General agreement',shared_term:'Shared relationship term',repair_standard:'Repair standard'})[kind]||humanize(kind||'general');}
...
function renderCouplesAgreements(state){
  ...
  const active=items.filter(x=>x.status==='agreed'&&!x.superseded_by);
  const activeRepair=active.filter(x=>x.kind==='repair_standard');
  const repairBox=$('couplesActiveRepairStandard');
  if(repairBox){
    if(activeRepair.length){repairBox.innerHTML=`<div class="repair-standard-title">Active mutually accepted repair standard${activeRepair.length>1?'s':''}</div>${activeRepair.map(x=>`<div class="agreement-item"><div><strong>v${Number(x.version||1)} — ${escapeHtml(x.text)}</strong><div class="muted">This version remains active unless both people accept a replacement.</div></div></div>`).join('')}`;show('couplesActiveRepairStandard',true);}
    else{repairBox.innerHTML='';show('couplesActiveRepairStandard',false);}
  }
  $('couplesAgreementList').innerHTML=items.length?items.map(x=>{
    const my=x.responses?.[me];const minePending=canAct&&!my;const parent=x.parent_id?byId[x.parent_id]:null;
    const isReplacement=Boolean(x.parent_id);
    const pendingChange=isReplacement&&x.status==='pending';
    const disputedChange=isReplacement&&x.status==='disputed';
    const lineage=isReplacement?`<div class="agreement-lineage">Version ${Number(x.version||1)} — proposed replacement for v${Number(parent?.version||Math.max(1,Number(x.version||1)-1))}: ${escapeHtml(parent?.text||'prior agreement')}</div>`:(x.superseded_by?`<div class="agreement-lineage">Version ${Number(x.version||1)} — superseded by a later mutually accepted version.</div>`:`<div class="agreement-lineage">Version ${Number(x.version||1)}</div>`);
    const guard=pendingChange?`<div class="agreement-proposal-warning">Proposed change only — the prior shared agreement remains active until both people agree.</div>`:(disputedChange?`<div class="agreement-proposal-warning">Replacement not mutually accepted — the prior shared agreement remains active.</div>`:'');
    return `<div class="agreement-item"><div><strong>${escapeHtml(x.text)}</strong><div class="agreement-meta"><span class="type-badge">${escapeHtml(agreementKindLabel(x.kind))}</span><span class="type-badge">v${Number(x.version||1)}</span><span class="type-badge">${escapeHtml(humanize(x.status||'pending'))}</span></div><div class="muted">Proposed by ${escapeHtml(state?.participants?.[x.proposed_by]?.name||x.proposed_by)}</div>${lineage}${guard}</div>${minePending?`<div class="agreement-actions"><button class="mini-btn agreement-response" data-id="${escapeHtml(x.id)}" data-decision="agree">Agree</button><button class="mini-btn agreement-response" data-id="${escapeHtml(x.id)}" data-decision="disagree">Disagree</button></div>`:''}</div>`;
  }).join(''):'<p class="muted">No shared agreement has been proposed yet.</p>';
  const parentSelect=$('couplesAgreementParent');
  if(parentSelect){const current=parentSelect.value;parentSelect.innerHTML='<option value="">No — this is a new agreement</option>'+active.map(x=>`<option value="${escapeHtml(x.id)}">Replace v${Number(x.version||1)}: ${escapeHtml(x.text.slice(0,100))}</option>`).join('');if([...parentSelect.options].some(o=>o.value===current))parentSelect.value=current;}
...
function renderCouplesOutcome(state){
  ...
  const closure=o.closure_agreement||{};const hist=o.current_behavior_vs_historical_injury||{};const defs=o.definition_collisions||[];
  $('couplesOutcome').innerHTML=`<div class="outcome-grid"><div><strong>What appears established / common ground</strong>${list(o.common_ground)}</div><div><strong>Still disputed / unresolved</strong>${list(o.still_disputed)}</div></div>${closure.status?`<div class="closure-agreement-card"><strong>Closure Agreement — ${escapeHtml(humanize(closure.status))}</strong><p><b>Proposed status:</b> ${escapeHtml(topicStatusLabel(closure.proposed_status||''))}</p><p><b>Resolved:</b> ${escapeHtml(closure.resolved_summary||'')}</p><p><b>Still open:</b> ${escapeHtml(closure.remains_open||'')}</p><p><b>Next step:</b> ${escapeHtml(closure.next_step||'')}</p><p><b>Repair:</b> ${escapeHtml(humanize(closure.repair_completion||'unclear'))}</p></div>`:''}${defs.length?`<h4>Definition collisions</h4>${defs.map(x=>`<div class="definition-collision-card"><strong>${escapeHtml(x.term||'Shared term')}</strong><p><b>A:</b> ${escapeHtml(x.participant_a_meaning||'')}</p><p><b>B:</b> ${escapeHtml(x.participant_b_meaning||'')}</p><p>${escapeHtml(x.assessment||'')}</p></div>`).join('')}`:''}${hist.applies?`<div class="history-split-card"><strong>Current behavior vs. historical injury</strong><p><b>Historical injury:</b> ${escapeHtml(hist.historical_injury||'')}</p><p><b>Current behavior:</b> ${escapeHtml(hist.current_behavior||'')}</p><span class="type-badge">${escapeHtml(humanize(hist.relationship||'unclear'))}</span><p>${escapeHtml(hist.assessment||'')}</p></div>`:''}${o.resolution_obstacle?`<div class="couples-relevance-summary"><strong>Possible resolution obstacle</strong><p>${escapeHtml(o.resolution_obstacle)}</p></div>`:''}${(o.active_repair_standards||[]).length?`<h4>Active mutually accepted repair standard</h4>${list(o.active_repair_standards)}`:''}${(o.pending_repair_changes||[]).length?`<div class="agreement-proposal-warning"><strong>Repair-standard change still requires mutual agreement</strong>${list((o.pending_repair_changes||[]).map(x=>`v${Number(x.version||1)} ${humanize(x.status)}: ${x.text}`))}</div>`:''}${(o.agreements_reached||[]).length?`<h4>Agreements reached</h4>${list(o.agreements_reached)}`:''}${(o.parked_topics||[]).length?`<h4>Issues saved for later</h4>${list(o.parked_topics)}`:''}${(o.suggested_next_questions||[]).length?`<h4>Questions to carry forward</h4>${list(o.suggested_next_questions)}`:''}${o.main_issue?`<div class="quote"><strong>Underlying issue:</strong> ${escapeHtml(o.main_issue)}</div>`:''}`;
}

async function proposeCouplesAgreement(){
  clearError();const text=($('couplesAgreementText').value||'').trim();if(!text)return;
  const kind=$('couplesAgreementKind')?.value||'general';const parent_id=$('couplesAgreementParent')?.value||'';
  try{const r=await fetch(`/api/couples/${encodeURIComponent(couplesSession.code)}/agreement/propose`,{method:'POST',headers:couplesHeaders(true),body:JSON.stringify({text,kind,parent_id})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to propose the agreement.');$('couplesAgreementText').value='';if($('couplesAgreementParent'))$('couplesAgreementParent').value='';renderCouplesState(d.state);}catch(e){error(e.message||String(e));}
}
...
async function closeCouplesTopic(){
  clearError();
  const body={status:$('couplesCloseStatus').value,resolved_summary:($('couplesCloseResolved').value||'').trim(),remains_open:($('couplesCloseOpen').value||'').trim(),next_step:($('couplesCloseNextStep').value||'').trim(),repair_completion:$('couplesCloseRepair').value};
  try{const r=await fetch(`/api/couples/${encodeURIComponent(couplesSession.code)}/topic/closure/propose`,{method:'POST',headers:couplesHeaders(true),body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to propose closure.');renderCouplesState(d.state);}catch(e){error(e.message||String(e));}
}

async function respondCouplesClosure(decision){
  clearError();
  try{const r=await fetch(`/api/couples/${encodeURIComponent(couplesSession.code)}/topic/closure/respond`,{method:'POST',headers:couplesHeaders(true),body:JSON.stringify({decision})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to record the closure response.');renderCouplesState(d.state);}catch(e){error(e.message||String(e));}
}
...
function printCouplesSession(){
  ...
  const insight=state.insights?.analysis;const summary=insight?`<section><h2>Conversation Insights</h2><p>${escapeHtml(insight.summary||'')}</p><h3>Common ground</h3>${list(insight.common_ground)}<h3>Still disputed</h3>${list(insight.disputed_points)}${(insight.definition_collisions||[]).length?`<h3>Definition collisions</h3>${(insight.definition_collisions||[]).map(x=>`<p><b>${escapeHtml(x.term||'Shared term')}:</b> A — ${escapeHtml(x.participant_a_meaning||'')} / B — ${escapeHtml(x.participant_b_meaning||'')}</p>`).join('')}`:''}</section>`:'';
  const closure=state.topic?.closure_agreement;const closureBlock=closure?`<section><h2>Closure Agreement</h2><p><b>Status:</b> ${escapeHtml(humanize(closure.status||'pending'))} • <b>Topic status:</b> ${escapeHtml(topicStatusLabel(closure.proposed_status||''))}</p><p><b>Resolved:</b> ${escapeHtml(closure.resolved_summary||'')}</p><p><b>Still open:</b> ${escapeHtml(closure.remains_open||'')}</p><p><b>Next step:</b> ${escapeHtml(closure.next_step||'')}</p><p><b>Repair:</b> ${escapeHtml(humanize(closure.repair_completion||'unclear'))}</p></section>`:'';
  const agreements=(state.agreements||[]).length?`<section><h2>Agreement Version History</h2><ul>${(state.agreements||[]).map(x=>`<li><b>${escapeHtml(agreementKindLabel(x.kind))} v${Number(x.version||1)}</b> — ${escapeHtml(x.text)} — ${escapeHtml(humanize(x.status||'pending'))}</li>`).join('')}</ul></section>`:'';
...
function renderCouplesState(state){
  ...
  show('couplesTextInviteBtn',couplesSession.participantId==='A'&&!b);
  show('couplesCopyInviteBtn',couplesSession.participantId==='A'&&!b);
  show('couplesInviteNote',couplesSession.participantId==='A'&&!b);
...
async function fetchCouplesState(silent=true){
  ...
  try{
    ...
    const d=await r.json();
    if(!r.ok){
      if(r.status===404||r.status===401){clearCouplesResume();if(couplesGuestInviteMode)showCouplesGuestGate(couplesGuestInviteCode||pendingCouplesInviteCode(),d.error||'This Couples Live session is no longer available.');else{renderCouplesLobby();if(!silent)error(d.error||'Couples Live room unavailable.');}}
...
async function joinCouplesSession(){
  ...
  try{
    ...
    const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to join the session.');
    couplesSession={...couplesSession,code:d.code,token:d.token,participantId:d.participant_id,name:d.state?.participants?.[d.participant_id]?.name||name,state:d.state,lastRevision:-1};
    try{sessionStorage.removeItem(COUPLES_INVITE_KEY);}catch{}
    try{const u=new URL(location.href);u.searchParams.delete('code');u.searchParams.delete('session');u.searchParams.delete('mode');history.replaceState({},document.title,u.pathname+(u.search||'')+(u.hash||''));}catch{}
    saveCouplesResume();renderCouplesState(d.state);startCouplesPoll();
  }catch(e){error(e.message||String(e));}
...
function leaveCouplesRoom(){
  ...
  clearCouplesResume();
  if(couplesGuestInviteMode){showCouplesGuestGate(couplesGuestInviteCode||pendingCouplesInviteCode(),'You left this Couples Live session on this device.');return;}
  renderCouplesLobby();
}
...
function switchMode(mode){
  if(couplesGuestInviteMode)mode='couples';
...
function renderConversationIntelligence(intel={}){
  ...
  const definitions=(intel.definition_collisions||[]).map(x=>`<div class="definition-collision-card"><strong>${escapeHtml(x.term||'Definition collision')}</strong>${(x.speaker_meanings||[]).map(m=>`<p>${escapeHtml(m)}</p>`).join('')}<p><b>Overlap:</b> ${escapeHtml(x.shared_overlap||'')}</p><p>${escapeHtml(x.why_it_matters||'')}</p><div class="quote">${escapeHtml(x.clarifying_question||'')}</div>${renderEvidenceRefs(x.evidence_turn_refs||[])}</div>`).join('');
  const history=(intel.current_vs_historical_patterns||[]).map(x=>`<div class="history-split-card"><strong>${escapeHtml(x.issue||'Issue')}</strong><p><b>Historical injury:</b> ${escapeHtml(x.historical_injury||'')}</p><p><b>Current behavior:</b> ${escapeHtml(x.current_behavior||'')}</p><span class="type-badge">${escapeHtml(humanize(x.relationship||'unclear'))}</span><p>${escapeHtml(x.assessment||'')}</p>${renderEvidenceRefs(x.evidence_turn_refs||[])}</div>`).join('');
  ...
  return `<section class="conversation-intelligence">
    ...
    ${definitions?`<details open><summary>Definition collisions</summary><div class="definition-collision-list">${definitions}</div></details>`:''}
    ${history?`<details open><summary>Current behavior vs. historical injury</summary><div class="history-split-list">${history}</div></details>`:''}
...
const SCREENSHOT_MAX_FILES=10;
const SCREENSHOT_MAX_SOURCE_BYTES=12*1024*1024;
const SCREENSHOT_MAX_DIMENSION=2200;
const SCREENSHOT_MINE_COLOR_KEY='clearsayScreenshotMineColor';
const SCREENSHOT_OTHER_COLOR_KEY='clearsayScreenshotOtherColor';
const SCREENSHOT_ALLOWED_COLORS=new Set(['gray','blue','green']);

function selectedScreenshotColor(name){
  const el=document.querySelector(`input[name="${name}"]:checked`);
  const value=String(el?.value||'').toLowerCase();
  return SCREENSHOT_ALLOWED_COLORS.has(value)?value:'';
}
function selectedScreenshotMineColor(){return selectedScreenshotColor('screenshotMineColor');}
function selectedScreenshotOtherColor(){return selectedScreenshotColor('screenshotOtherColor');}
function colorLabel(color){return color?color[0].toUpperCase()+color.slice(1):'';}
function updateScreenshotColorStatus(){
  const mine=selectedScreenshotMineColor(), other=selectedScreenshotOtherColor();
  const status=$('screenshotColorStatus');
  let message='Choose both message colors before reading screenshots.';
  let bad=false;
  if(mine&&other){
    if(mine===other){message='Your messages and their messages cannot use the same selected color for automatic speaker assignment.';bad=true;}
    else message=`${colorLabel(mine)} = You • ${colorLabel(other)} = Other person`;
  }else if(mine)message=`${colorLabel(mine)} = You. Now choose their message color.`;
  else if(other)message=`${colorLabel(other)} = Other person. Now choose your message color.`;
  if(status){status.textContent=message;status.classList.toggle('color-status-error',bad);}
  if(mine){try{localStorage.setItem(SCREENSHOT_MINE_COLOR_KEY,mine);}catch{}}
  if(other){try{localStorage.setItem(SCREENSHOT_OTHER_COLOR_KEY,other);}catch{}}
}
function restoreScreenshotColors(){
  let mine='',other='';
  try{mine=String(localStorage.getItem(SCREENSHOT_MINE_COLOR_KEY)||'').toLowerCase();other=String(localStorage.getItem(SCREENSHOT_OTHER_COLOR_KEY)||'').toLowerCase();}catch{}
  if(SCREENSHOT_ALLOWED_COLORS.has(mine)){const el=document.querySelector(`input[name="screenshotMineColor"][value="${mine}"]`);if(el)el.checked=true;}
  if(SCREENSHOT_ALLOWED_COLORS.has(other)){const el=document.querySelector(`input[name="screenshotOtherColor"][value="${other}"]`);if(el)el.checked=true;}
  updateScreenshotColorStatus();
}

function screenshotFileLabel(file,index){return file?.name||`Screenshot ${index+1}`;}
function clearScreenshotObjectUrls(){for(const item of screenshotUploads){if(item.previewUrl)URL.revokeObjectURL(item.previewUrl);}}
function resetScreenshotUploads(){
  clearScreenshotObjectUrls(); screenshotUploads=[];
  if($('screenshotInput'))$('screenshotInput').value='';
  if($('screenshotPreviewList')){$('screenshotPreviewList').innerHTML='';show('screenshotPreviewList',false);}
  if($('screenshotStatus'))$('screenshotStatus').textContent='';
  show('screenshotActions',false);
}
function renderScreenshotUploads(){
  const list=$('screenshotPreviewList'); if(!list)return;
  if(!screenshotUploads.length){resetScreenshotUploads();return;}
  list.innerHTML=screenshotUploads.map((item,i)=>`<article class="screenshot-preview-card"><div class="screenshot-thumb-wrap"><img src="${escapeHtml(item.previewUrl)}" alt="Preview of ${escapeHtml(screenshotFileLabel(item.file,i))}"></div><div class="screenshot-preview-meta"><strong>${i+1}. ${escapeHtml(screenshotFileLabel(item.file,i))}</strong><span>${Math.max(1,Math.round((item.file?.size||0)/1024)).toLocaleString()} KB</span></div><button type="button" class="mini-btn screenshot-remove" data-index="${i}" aria-label="Remove screenshot ${i+1}">Remove</button></article>`).join('');
  show('screenshotPreviewList',true);show('screenshotActions',true);
  if($('screenshotStatus'))$('screenshotStatus').textContent=`${screenshotUploads.length} screenshot${screenshotUploads.length===1?'':'s'} ready • kept in this browser only until you remove or refresh them.`;
  document.querySelectorAll('.screenshot-remove').forEach(btn=>btn.addEventListener('click',()=>{
    const i=Number(btn.dataset.index);const [removed]=screenshotUploads.splice(i,1);if(removed?.previewUrl)URL.revokeObjectURL(removed.previewUrl);renderScreenshotUploads();
  }));
}
function handleScreenshotSelection(event){
  ...
  for(const f of incoming){
    if(!allowed.has(f.type))rejected.push(`${f.name}: unsupported format`);
    else if(f.size>SCREENSHOT_MAX_SOURCE_BYTES)rejected.push(`${f.name}: over 12 MB`);
    else accepted.push(f);
  }
  const room=Math.max(0,SCREENSHOT_MAX_FILES-screenshotUploads.length);
  const add=accepted.slice(0,room);
  if(accepted.length>room)rejected.push(`Only ${SCREENSHOT_MAX_FILES} screenshots can be imported at once.`);
  for(const file of add)screenshotUploads.push({file,previewUrl:URL.createObjectURL(file)});
  event.target.value='';renderScreenshotUploads();
  if(rejected.length)error(rejected.join(' • '));
}
async function screenshotFileToDataUrl(file){
  const sourceUrl=URL.createObjectURL(file);
  try{
    const img=await new Promise((resolve,reject)=>{const x=new Image();x.onload=()=>resolve(x);x.onerror=()=>reject(new Error(`Could not read ${file.name}.`));x.src=sourceUrl;});
    const scale=Math.min(1,SCREENSHOT_MAX_DIMENSION/Math.max(img.naturalWidth||1,img.naturalHeight||1));
...
async function readScreenshotsIntoTranscript(){
  clearError();if(!screenshotUploads.length){error('Add at least one screenshot first.');return;}
  const mineColor=selectedScreenshotMineColor();
  const otherColor=selectedScreenshotOtherColor();
  if(!mineColor||!otherColor){error('Choose the message-bubble color for both you and the other person before reading the screenshots.');$('screenshotColorFieldset')?.scrollIntoView({behavior:'smooth',block:'center'});return;}
  if(mineColor===otherColor){error('Choose two different message colors so ClearSay can assign the speakers reliably.');$('screenshotColorFieldset')?.scrollIntoView({behavior:'smooth',block:'center'});return;}
  const btn=$('readScreenshotsBtn');if(btn){btn.disabled=true;btn.textContent='Reading screenshots…';}
  if($('screenshotStatus'))$('screenshotStatus').textContent='Preparing screenshots for text extraction…';
  try{
    const images=[];
    for(let i=0;i<screenshotUploads.length;i++){
      if($('screenshotStatus'))$('screenshotStatus').textContent=`Preparing screenshot ${i+1} of ${screenshotUploads.length}…`;
      images.push({name:screenshotFileLabel(screenshotUploads[i].file,i),data_url:await screenshotFileToDataUrl(screenshotUploads[i].file)});
    }
    if($('screenshotStatus'))$('screenshotStatus').textContent='Reading visible conversation text…';
    const r=await fetch('/api/transcript/screenshots',{method:'POST',headers:{'Content-Type':'application/vnd.clearsay.screenshots'},body:JSON.stringify({images,mine_color:mineColor,other_color:otherColor})});
    const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to read the screenshots.');
    const extracted=String(d.transcript||'').trim();if(!extracted)throw new Error('No conversation text could be read from these screenshots.');
    const existing=$('transcriptText').value.trim();$('transcriptText').value=existing?`${existing}\n\n${extracted}`:extracted;
    transcriptSession.transcript=$('transcriptText').value;saveTranscriptSessionState();
    const warnings=Array.isArray(d.warnings)&&d.warnings.length?` • ${d.warnings.join(' • ')}`:'';
    if($('screenshotStatus'))$('screenshotStatus').textContent=`Screenshot text added. ${colorLabel(mineColor)} = You • ${colorLabel(otherColor)} = Other person. Review speaker labels and wording before analysis${warnings}`;
    $('transcriptText').focus();$('transcriptText').scrollIntoView({behavior:'smooth',block:'center'});
  }catch(e){error(e.message||String(e));if($('screenshotStatus'))$('screenshotStatus').textContent='Screenshot import did not complete.';}
  finally{if(btn){btn.disabled=false;btn.textContent='Read screenshots into transcript';}}
}
...
async function startQualification(){
  ...
  try{
    ...
    if(session.source==='statement')collapseStatementPanelForUpdate('qualification');
...
async function runAI(isFollowup=false){
  ...
  try{
    ...
    if(session.source==='live'){
      ...
      renderQuestions(d.analysis.questions);show('clarification',true);collapseStatementPanelForUpdate('clarification');$('clarification').scrollIntoView({behavior:'smooth',block:'start'});
    } else {show('clarification',false);collapseStatementPanelForUpdate('analysis');$('results').scrollIntoView({behavior:'smooth',block:'start'});}
  }catch(e){error(e.message||String(e));}
...
async function copyReplyText(text,button){
  ...
  if(!clean){error('There is no reply to copy yet.');return;}
  let copied=false;
  try{
    if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(clean);copied=true;}
  }catch{}
  if(!copied)copied=fallbackCopyText(clean);
  if(!copied){window.prompt('Copy this reply:',clean);return;}
  if(button){
    const original=button.dataset.copyLabel||button.textContent||'Copy reply';
...
function copySelectedLiveReply(button){
  if(session.source!=='live'||!session.liveTurnId)return;
  const turn=liveTurnById(session.liveTurnId);if(!turn)return;
  const key=turn.selected_response_key||'best';
  copyReplyText(liveResponseText(turn,key,turn.response_target||'other_person'),button);
}

function copyGeneralReply(key='best',button){
  const a=session.last?.analysis;if(!a)return;
  const set=responseSetForAnalysis(a,generalResponseTarget);
  copyReplyText(set[key]||'',button);
}
...
function renderResults(a,meta){
  ...
  const definition=a.definition_collision||{};
  const definitionSection=definition.applies?`<section class="card full definition-collision-card"><div class="section-kicker">DEFINITION COLLISION</div><h2>${escapeHtml(definition.term||'Same word, different meaning')}</h2><div class="result-grid"><div><strong>Meaning A</strong><p>${escapeHtml(definition.meaning_a||'Unclear')}</p></div><div><strong>Meaning B</strong><p>${escapeHtml(definition.meaning_b||'Unclear')}</p></div></div><p><strong>Overlap:</strong> ${escapeHtml(definition.shared_overlap||'')}</p><p>${escapeHtml(definition.assessment||'')}</p><div class="quote">${escapeHtml(definition.clarifying_question||'')}</div></section>`:'';
  const currentHistorical=a.current_vs_historical||{};
  const currentHistorySection=currentHistorical.applies?`<section class="card full history-split-card"><div class="section-kicker">CURRENT BEHAVIOR ≠ HISTORICAL INJURY</div><h2>${escapeHtml(humanize(currentHistorical.relationship||'unclear'))}</h2><div class="result-grid"><div><strong>Historical injury</strong><p>${escapeHtml(currentHistorical.historical_injury||'Unclear')}</p></div><div><strong>Current behavior</strong><p>${escapeHtml(currentHistorical.current_behavior||'Unclear')}</p></div></div><p>${escapeHtml(currentHistorical.assessment||'')}</p></section>`:'';
  ...
  const liveQuickPanel=session.source==='live'?`<section class="card full live-quick-response-card"><div class="section-kicker">QUICK LIVE RESPONSE</div>${liveResponseTargetSwitch}<h2 id="liveSelectedResponseTitle">${selectedLiveKey==='best'?'Best next response':`Selected response — ${escapeHtml(liveResponseLabel(selectedLiveKey))}`} to ${escapeHtml(responseTargetLabel(selectedLiveTarget))}</h2><div id="liveSelectedResponseText" class="quote">${escapeHtml(selectedLiveText)}</div><div class="reply-copy-row"><button id="liveCopyResponseBtn" class="mini-btn copy-reply-btn" type="button">Copy reply</button><span class="muted">Paste it into Messages or your texting app.</span></div>${liveResponseChoices}${liveResponseControls}</section>`:'';
  const generalSet=responseSetForAnalysis(a,generalResponseTarget);
  const generalResponsePanel=session.source==='live'?'':`<section class="card full response-panel"><div class="section-kicker">RESPONSE</div><h2>What to say next</h2><div class="response-target-wrap"><div class="live-response-prompt"><strong>Respond to</strong><span class="muted">Switch the audience without rerunning the analysis.</span></div><div class="response-target-switch"><button type="button" class="response-target-btn general-response-target ${generalResponseTarget==='other_person'?'selected':''}" data-response-target="other_person">Other person</button><button type="button" class="response-target-btn general-response-target ${generalResponseTarget==='therapist'?'selected':''}" data-response-target="therapist">Therapist</button></div></div><p class="muted">Recommended wording to <strong id="generalResponseAudience">${escapeHtml(responseTargetLabel(generalResponseTarget))}</strong></p><div id="generalResponseQuote" class="quote">${escapeHtml(generalSet.best)}</div><div class="reply-copy-row"><button id="generalCopyResponseBtn" class="mini-btn copy-reply-btn" type="button">Copy reply</button><span class="muted">Paste it into Messages or your texting app.</span></div><details class="response-alternatives-details"><summary>Alternative responses</summary><div class="live-response-choice-grid"><div class="response-alt-card"><strong>Clarify</strong><span id="generalResponse-clarify">${escapeHtml(generalSet.clarify)}</span><button type="button" class="mini-btn copy-general-option" data-response-key="clarify">Copy</button></div><div class="response-alt-card"><strong>Evidence</strong><span id="generalResponse-evidence">${escapeHtml(generalSet.evidence)}</span><button type="button" class="mini-btn copy-general-option" data-response-key="evidence">Copy</button></div><div class="response-alt-card"><strong>Direct</strong><span id="generalResponse-direct">${escapeHtml(generalSet.direct)}</span><button type="button" class="mini-btn copy-general-option" data-response-key="direct">Copy</button></div><div class="response-alt-card"><strong>De-escalate</strong><span id="generalResponse-deescalating">${escapeHtml(generalSet.deescalating)}</span><button type="button" class="mini-btn copy-general-option" data-response-key="deescalating">Copy</button></div></div></details></section>`;
  ...
  if(session.source==='live'){
    ...
    $('liveCopyResponseBtn')?.addEventListener('click',(e)=>copySelectedLiveReply(e.currentTarget));
  ...
  $('generalCopyResponseBtn')?.addEventListener('click',(e)=>copyGeneralReply('best',e.currentTarget));
  document.querySelectorAll('.copy-general-option').forEach(btn=>btn.addEventListener('click',(e)=>copyGeneralReply(btn.dataset.responseKey,e.currentTarget)));
  $('liveCopyResponseBtn')?.addEventListener('click',(e)=>copySelectedLiveReply(e.currentTarget));
  $('liveUsedResponseBtn')?.addEventListener('click',()=>markLiveResponse('used'));
  $('liveModifiedResponseBtn')?.addEventListener('click',()=>markLiveResponse('modified'));
  $('liveDidNotUseResponseBtn')?.addEventListener('click',()=>markLiveResponse('not_used'));
...
function reset(){generalResponseTarget='other_person';session={claim:'',context:'',round:0,answers:[],last:null,route:null,selectedClaimTypes:[],source:'statement',liveTurnId:null};$('claim').value='';$('context').value='';$('questionList').innerHTML='';$('claimQuestionList').innerHTML='';$('detectedClaimTypes').innerHTML='';$('results').innerHTML='';show('results',false);show('clarification',false);show('claimQualification',false);setStatementUpdateNotice('');openStatementPanel();clearError();window.scrollTo({top:0,behavior:'smooth'});}
...
function initializeClearSayApp(){
  ...
  const hasPendingCouplesInvite=Boolean(pendingCouplesInviteCode());
  ...
  $('couplesTextInviteBtn')?.addEventListener('click',textCouplesInvite);
  $('couplesCopyInviteBtn')?.addEventListener('click',copyCouplesInvite);
  ...
  document.querySelectorAll('input[name="screenshotMineColor"], input[name="screenshotOtherColor"]').forEach(el=>el.addEventListener('change',updateScreenshotColorStatus));
  restoreScreenshotColors();
  $('readScreenshotsBtn')?.addEventListener('click',readScreenshotsIntoTranscript);
  $('clearScreenshotsBtn')?.addEventListener('click',resetScreenshotUploads);
  ...
  if(hasPendingCouplesInvite && (!couplesGuestInviteMode || savedCouplesResumeMatches(pendingCouplesInviteCode())))applyPendingCouplesInvite();
}

document.addEventListener('DOMContentLoaded',()=>{
  captureCouplesInviteFromUrl();
  initTheme();
  $('legalAcknowledgeBtn')?.addEventListener('click',acknowledgeLegalNotice);
  $('checkoutBtn')?.addEventListener('click',beginCheckout);
  $('paymentRefreshBtn')?.addEventListener('click',refreshPaymentAccess);
  $('couplesGuestJoinBtn')?.addEventListener('click',joinCouplesGuestSession);
  if(couplesGuestInviteMode){
    const code=pendingCouplesInviteCode();
    if(code&&savedCouplesResumeMatches(code)){document.body.classList.add('couples-guest-session');showClearSayApp();switchMode('couples');}
    else showCouplesGuestGate(code);
    return;
  }
  setLegalGate(false);
});
