...
async function checkHealth(){try{const r=await fetch('/api/health');const d=await r.json();$('health').textContent=d.ai_configured?`AI connected • ${d.model} • ${d.fallacies} traps • local keyword-first routing`:'Server online • API key not configured';}catch{$('health').textContent='Server connection unavailable';}}
