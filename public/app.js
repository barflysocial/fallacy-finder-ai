...
const COUPLES_SESSION_KEY='fallacyFinderCouplesLiveV150';
...
// -----------------------------------------------------------------------------
// Couples Live v1.5.0 — shared post-send reasoning room
// -----------------------------------------------------------------------------
...
function renderCouplesTopic(state){
  ...
  const composerLocked=state?.viewer_role==='mediator'||Boolean(state?.session_ended_at)||topic.status!=='open'||!state?.participants?.B;
  $('couplesMessage').disabled=composerLocked;
  $('couplesSendBtn').disabled=composerLocked;
  if(state?.viewer_role==='mediator'){
    $('couplesMessage').placeholder='Therapist / mediator access is read-only.';
  }else if(state?.session_ended_at){
...
async function continueCouplesIssue(){
  clearError();if(!window.confirm('Start a new 15-minute room carrying this unresolved issue, common ground, agreements, and open questions forward?'))return;
...
function renderCouplesState(state){
  ...
  $('couplesParticipants').textContent=(b?`${a?.name||'Person A'} and ${b.name}`:`${a?.name||'Person A'} • waiting for Person B to join`)+med;
  ...
  if(latest||state.session_ended_at){
    ...
    $('couplesCurrentInsight').innerHTML=`${state.session_ended_at?'<div class="couples-session-ended">15-minute session complete. Messaging is closed; review the conversation, build insights, or export the session.</div>':''}${latest?.shared_analysis?.underlying_issue?`<strong>Current underlying issue:</strong> ${escapeHtml(latest.shared_analysis.underlying_issue)}${effect.explanation?`<div class="muted"><strong>Latest effect:</strong> ${escapeHtml(effect.explanation)}</div>`:''}`:''}`;
...
async function createCouplesSession(){
  ...
  try{
    const r=await fetch('/api/couples/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,topic,resolution_criteria:resolutionCriteria,consent:true})});
...
async function joinCouplesSession(){
  ...
  try{
    const r=await fetch('/api/couples/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,name,consent:true})});
...
async function endCouplesSessionEarly(){
  if(!window.confirm('End the active 15-minute session now? The conversation will remain available for review and export.'))return;
